...
class CandidateProfile:
    def __init__(self, report_path: str):
        ...
        global weakness 
        weakness = self.report['weakness_analysis']
...
class LlamaNegotiationExpert:
    ...
    def _generate_prompt(self, negotiation_context: Dict) -> str:
        templates = {
            'initial': (
                "{candidate} is applying for {role}. Strengths: {strengths}. Weaknesses: {weaknesses}. "
                "Offered: {offer} {currency} (market: {market_min}-{market_max} {currency}). "
                "Candidate said: '{candidate_reaction}'. "
                "Defend the offer clearly. Emphasize the weaknesses. No unnecessary concessions."
            ),
            'counter': (
                "{candidate} demands {requested_amount} {currency} and asks for: {benefit_requests_str}. "
                "Offer: {offer} {currency}. Market range: {market_min}-{market_max} {currency}. "
                "Sentiment: {sentiment}. Bluff chance: {bluff_prob}. Weaknesses: {weaknesses}. "
                "Give a new adjusted  salary for the user based on his experience and his skills."
                "Always mention a offer price to user by his skills based on {offer}"
                "Reject exaggeration. Justify the cap. Give one firm alternative. No unnecessary details."
            ),
            'exploring_benefits': (
                "{candidate} is now negotiating benefits: {benefit_requests_str}. Salary remains {offer} {currency}. "
                "Weaknesses: {weaknesses}. "
                "Offer specific development perks if justified (e.g., training, mentorship). Avoid suggesting salary changes."
            ),
            'final': (
                "Final offer for {candidate}: {offer} {currency}, benefits: {final_benefits}. "
                "Decision based on market, profile, and weaknesses: {weaknesses}. "
                "This is final. Expect a clear yes or no. Do not re-engage."
            )
        }

        if self.flag:
            global weakness
            self.gaps = weakness.get(negotiation_context.get('role', {}), {})
            temp = self.gaps.get('technical_gaps', []) + self.gaps.get("experience_gaps", [])
            for i in temp:
                self.weak += i + ", "
            self.flag = False
        ...
        context_data = {
            ...
            'strengths': "relevant experience and technical skills",
            'weaknesses': self.weak,
        }

        state = negotiation_context.get('state')

        if not self.context_window:
            return templates['initial'].format(**context_data) + " Keep the tone strict. . Focus only on weaknesses."
        elif state == NegotiationState.FINAL_OFFER:
            return templates['final'].format(**context_data) + " No further negotiation. Avoid any polite closure or soft phrases."
        elif state == NegotiationState.EXPLORING_BENEFITS:
            return templates['exploring_benefits'].format(**context_data) + " Be concise. No emotional or corporate tone."
        else:
            return templates['counter'].format(**context_data) + " Stay factual. Don’t overexplain or soften the message.sUGGEST A OFFER  PRICE WITH HR'S PREVIOUS SALARY RESPONSE "
